...
const Navbar = () => {
  ...
  return (
    ...
    border-b border-borderColor relative transition-all ${location.pathname === "/" && "bg-light"}`}>
        ...
        border-bordercolor right-0 flex flex-col sm:flex-row item-start sm:items-center gap-4 sm:gap-8
        ...
        ${open ? "max-sm:translate-x-0" : "max-sm:translate-x-full"}`}>
            ...
            <div className='hidden lg:flex item-center text-sm gap-2 border border-borderColor px-3 
            rounded-full max-w-56'>
                <input type="text" className="py-1.5 w-full bg-tranparent outline-none placeholder-grey-500"
            ...
            <div className='flex max-sm:flex-col item-start sm:items-center gap-6'>
                ...
                <button onClick={()=> {user ? logout() : setShowLogin(true)}} className="cursor-pointer px-8 py-2 bg-primary hower:bg-primary-dull
                transition-all text-white rounded-lg">{user ? 'Logout' : 'Login'}</button>
            </div>
