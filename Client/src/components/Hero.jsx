...
const Hero = () => {
    ...
    const [pickupupLocation, setpickupupLocation] = useState('')

    const {pickupDate, setPickupDate, returnDate, setReturnDate, navigate} = useAppContext()

    const handleSearch = (e)=> {
        e.preventDefault()
        navigate('/cars?pickupLocation=' + pickupupLocation + '&pickupDate=' + pickupDate
  ...
  return (
    ...
    className='h-screen flex flex-col items-center justify-center gap-14 bg-light text-center'>
            ...
            translate={{duration: 0.8, delay: 0.2}} 
            className='text-4xl md:text-5xl font-semibold'>Affordable Cars On Rent</motion.h1>
        ...
        rounded-lg md:rounded-full w-full max-w-80 md:max-w-200 bg-white 
        shadow-[0px_8px_20px_rgba(0,0,0,0.1)]'>

            <div className='flex flex-col md:flex-row items-start md:items-center gap-10 min-md:ml-8'>
                <div className='flex flex-col items-start gap-2'>
                    <select required value={pickupupLocation} onChange={(e)=> setpickupupLocation(e.target.value)}>
                        <option value="">Pickup Location</option>
                        {cityList.map((city)=> <option key={city} value={city}>{city}</option>)}
                    </select>
                    <p className='px-1 text-sm text-gray-500'>
                        {pickupupLocation ? pickupupLocation : 'Please Select Location'}</p>
                </div>
                ...
                <div className='flex flex-col items-start gap-2'>
                    ...
                    className='text-sm text-grey-500' required/>
                </div>
