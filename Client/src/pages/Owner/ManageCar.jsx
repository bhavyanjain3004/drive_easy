...
//           <tbody>
//             {cars.map((car, index)=> (
//               <tr key={index} className='border-t border border-gray-300'>
...
//                   <img src={car.isAvaliable ? assets.eye_close_icon : assets.eye_icon} alt="Toggle visibility" className='cursor-pointer'/>
...
const ManageCar = () => {
  ...
  const toggleAvailability = useCallback(async (carId) => {
    try {
    ...
    <div className='px-4 pt-10 md:px-10 w-full'>
      ...
      <div className='max-w-3xl w-full rounded-md overflow-hidden border border-gray-300 mt-6'>
        <table className='w-full border-collapse text-left text-sm text-gray-600'>
          ...
          <tbody>
            {cars.map(car => (
              <tr key={car._id} className='border-t border border-gray-300'>
                <td className='p-3 flex items-center gap-3'>
                  <img src={car.image} alt="" className='h-12 w-12 aspect-square rounded-md object-cover'/>
                  <div className='max-md:hidden'>
                    <p className='font-medium'>{car.brand} {car.model}</p>
                ...
                <td className='flex items-center p-3 gap-2'>
                  <img onClick={()=> toggleAvailability(car._id)} src={car.isAvailable ? assets.eye_close_icon : assets.eye_icon} alt="Toggle visibility" className='cursor-pointer'/>
                  <img onClick={()=> deleteCar(car._id)} src={assets.delete_icon} alt="Delete" className='cursor-pointer'/>
                </td>
              </tr>
            ))}
          </tbody>
